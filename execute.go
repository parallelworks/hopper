...
func (c *Client[TTx]) execute(ctx context.Context, row *driver.JobRow, qcfg QueueConfig) driver.JobFinalize {
	unit, err := c.runWorker(ctx, row)
	return c.outcome(ctx, row, qcfg, unit, err)
}
...
func safeWork(ctx context.Context, unit workUnit) (err error) {
	defer func() {
		if r := recover(); r != nil {
...
// when the job never reached a worker.
func (c *Client[TTx]) outcome(ctx context.Context, row *driver.JobRow, qcfg QueueConfig, unit workUnit, err error) driver.JobFinalize {
	...
	case ctx.Err() != nil:
		// The client is stopping and cancelled the job. That is not the job's
		// fault: retry immediately elsewhere.
